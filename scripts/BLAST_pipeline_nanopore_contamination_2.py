...
ANNOTATION_PKL = "annotations_partial.pkl"
ANNOTATION_TSV = "annotations_partial.tsv"
ANNOTATED_DIRNAME = "annotated_tables"
ANNOTATION_CHECKPOINT_EVERY = 500
...
def tmp_tsv_path(path: str) -> str:
    return f"{path}.tmp"


def cleanup_stale_tmp_file(path: str) -> None:
    tmp_path = tmp_tsv_path(path)
    if os.path.exists(tmp_path):
        logging.warning(f"Removing stale temporary file: {tmp_path}")
        os.remove(tmp_path)
...
def build_empty_annotation(acc: str) -> Dict[str, str]:
    return {
        "Accession": acc,
        "Description": "",
        "Taxonomy": "",
...
def save_annotation_checkpoint(annotations: List[Dict[str, str]]) -> None:
    ann_df = pd.DataFrame(annotations)
    ann_df.to_pickle(ANNOTATION_PKL)
    ann_df.to_csv(ANNOTATION_TSV, sep="\t", index=False)
    ...
    try:
        if SEARCH_TARGET:
            ...
            if best:
                ...
                append_tsv_row(tmp_path, row)
            else:
                append_tsv_row(tmp_path, ["NA","NA","NA","No_match_found","NA","","","NA","NA","NA","NA","NA",str(len(seq)),"NA","NA","NA"])
        ...
        if os.path.exists(tmp_path):
            logging.warning(f"Temporary file left for retry debugging: {tmp_path}")
        raise
...
if os.path.exists(PARTIAL_PKL) and not args.force_reannotate:
    ...
    for fname in os.listdir(OUTPUT_DIR):
        ...
        if "Accession" not in df.columns:
            for alt in ["Accession_x", "Accession_y"]:
    ...
    # Normalize reruns: previous runs may have left _x/_y columns or step-2 columns in the TSVs
    for base in ["Definition", "Organism", "Taxonomy", "Release Year"]:
        if base not in all_hits.columns:
            for alt in [f"{base}_x", f"{base}_y"]:
                if alt in all_hits.columns:
                    all_hits[base] = all_hits[alt]
                    break

    if "Accession" not in all_hits.columns:
        for alt in ["Accession_x", "Accession_y"]:
            if alt in all_hits.columns:
                all_hits["Accession"] = all_hits[alt]
                break
    ...
    cols_to_drop += [c for c in all_hits.columns if re.search(r"_(x|y)$", c)]
    all_hits = all_hits.drop(columns=cols_to_drop, errors="ignore")

    # Unique valid accessions
    accs = sorted({a.split("|")[-1] for a in all_hits["Accession"] if isinstance(a, str) and a and a != "NA"})

    if os.path.exists(ANNOTATION_PKL) and not args.force_reannotate:
        ann_ckpt = pd.read_pickle(ANNOTATION_PKL)
        if ann_ckpt.empty:
            annotations = []
            done_accs = set()
        else:
            ann_ckpt = ann_ckpt.fillna("")
            annotations = ann_ckpt.to_dict(orient="records")
            done_accs = set(ann_ckpt["Accession"].astype(str))
        print(f"[INFO] Resuming annotation checkpoint: {ANNOTATION_PKL} ({len(done_accs)} accessions)")
    else:
        annotations = []
        done_accs = set()
    ...
    for i, acc in enumerate(accs, 1):
        if acc in done_accs:
            print(f"  [{i}/{len(accs)}] Fetching metadata for {acc}... SKIP")
            continue
        ...
        if not xml_data:
            print(" FAIL")
            not_found.add(acc)
            annotations.append(build_empty_annotation(acc))
            if len(annotations) % ANNOTATION_CHECKPOINT_EVERY == 0:
                save_annotation_checkpoint(annotations)
            time.sleep(DELAY)
            continue
    ...
    ann_df = pd.DataFrame(annotations).fillna("")
    save_annotation_checkpoint(annotations)
    ...
    # Persist enriched TSVs to a dedicated output directory instead of overwriting raw inputs
    annotated_dir = os.path.join(OUTPUT_DIR, ANNOTATED_DIRNAME)
    os.makedirs(annotated_dir, exist_ok=True)
    for sf, sub in all_hits.groupby("Source_File", dropna=False):
        out_tsv = os.path.join(annotated_dir, f"{sf}.annotated.tsv")
...
with pd.ExcelWriter(FINAL_EXCEL, engine="openpyxl") as writer:
    ...
    meta_cols = [c for c in [
        "Accession", "Description", "Taxonomy_y", "Taxonomy", "Accession Length",
        "Sequencing Technology", "Molecule Type", "Topology", "Assembly", "Release Year_y", "Release Year"
    ...
    if os.path.exists(PARTIAL_PKL):
        taxonomy_step2 = all_hits.get("Taxonomy_y", all_hits.get("Taxonomy", "")).astype(str)
        missing_meta = set(
            all_hits.loc[
